...
fn host() -> String {
    ...
    String::from_utf8_lossy(unsafe {
        CStr::from_ptr(utsname.nodename.as_ptr() as *const c_char).to_bytes()
    })
    .to_string()
}
...
fn fetch_packages_flatpak() -> Result<String, std::io::Error> {
    let mut count = std::fs::read_dir("/var/lib/flatpak/app")?
        .into_iter()
        .filter(|entry| entry.as_ref().unwrap().path().is_dir())
        .count();
    count += std::fs::read_dir("/var/lib/flatpak/runtime")?
        .into_iter()
        .filter(|entry| entry.as_ref().unwrap().path().is_dir())
        .count();
    Ok(count.to_string())
